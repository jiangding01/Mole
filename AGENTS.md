...
- `cmd/analyze/` - Go disk-analysis TUI. `main.go` is bootstrap only; `model.go` holds types and accessor methods; `update.go` holds the Bubble Tea Update chain. `serve.go` is the `--serve` GUI engine (NDJSON scan/children/rescan/cancel over stdio, session cache, live scan reuse).
- `bin/robot.sh` + `lib/core/robot.sh` - robot machine mode (protocol v1 NDJSON, docs/MAC_APP_DESIGN.md §4) that the Mac app drives: clean/apps plan+apply, optimize, launchitems, updates, history, whitelist. Destructive verbs take plan-file ids, never caller paths; apply re-verifies every id through exists → protect → whitelist → `mole_delete`. Clean-scan liveness comes from polling the NUL-delimited preview ledger (`MOLE_CLEAN_PREVIEW_LEDGER_FILE`), not the export file; guard-skipped families flow through the same injection pattern (`MOLE_CLEAN_DEFERRED_FILE` → `guard_skipped` insight events, byte counts never promised). Run `MOLE_TEST_NO_AUTH=1 bats tests/robot_core.bats tests/robot_apps_plan.bats tests/robot_optimize.bats tests/robot_launchitems.bats tests/robot_updates.bats`.
- `app/` - SwiftUI Mac app (`MoleApp` features + `MoleKit` SwiftPM package; `xcodegen generate` builds the project, scheme name is `Mole`). Features/ must not spawn processes or delete files directly — everything routes MoleKit → `mole robot` → the CLI safety chain (SwiftLint custom rules enforce this). See `app/README.md` and `docs/MAC_APP_DESIGN.md`.
- `contracts/robot_v1/` - golden NDJSON files consumed by both CLI bats and MoleKit `GoldenContractTests`; changing event shapes means updating goldens on both ends in the same change.
- `design/mole-dc/Mole.dc.html` - the high-fidelity design prototype. Visual/interaction questions are settled by running it in a browser, not by debate; `HANDOFF.md` beside it is required reading before UI work. `support.js` is the prototype runtime and is never ported.
...
make build
go test ./...
MOLE_TEST_NO_AUTH=1 bats tests/robot_core.bats tests/robot_apps_plan.bats   # robot layer
MOLE_TEST_NO_AUTH=1 ./mole robot clean plan                                 # read-only e2e smoke
cd app/MoleKit && swift test                                                # MoleKit unit + contract tests
cd app && xcodegen generate && xcodebuild -project Mole.xcodeproj -scheme Mole -configuration Debug build CODE_SIGNING_ALLOWED=NO
```
...
- **A path with an embedded newline poisons every line-delimited format it crosses.** A real directory named `$'mole\n,'` rendered two export lines whose first fragment was byte-identical to the real `~/Library/Logs/mole` — the plan then carried a path clean never proposed. Writers of line formats must reject `\n`/`\r` paths (`render_clean_preview_from_ledger`), parsers must reject marker-less lines (`robot_clean_plan_from_export`), and no tooling may `head -1` or line-split such a path: even the cleanup one-liner truncated it into its real sibling. Enumerate with `-print0` or a list API, never text lines.
- **`printf 'a\0100'` is an octal escape, not NUL+"100".** When writing NUL-delimited fixtures or ledgers in bash, emit each field as `printf '%s\0' "$field"`; inline `\0` followed by digits silently corrupts the byte stream (hit writing the robot ledger bats fixtures).
...
## Mac App and Robot Layer

Invariants for `app/` and the robot machine mode. Every one of these is a shipped bug or a hard contract, not style preference.

- **Safety chain is structural**: Features/ never contains `Process(`, `FileManager…removeItem`, or `NSWorkspace…recycle` (SwiftLint custom rules fail the build). Deletion flows MoleKit → `mole robot <domain> apply` with plan-file **ids, never paths**; apply re-verifies every id through exists → protect → whitelist → `mole_delete` (Trash mode).
- **Page Stores live at session level** (`RootView` `@State`, injected via `.environment`). Hanging a Store on a page's own `@State` destroys it on tab switch: in-flight scans die, re-entry rescans, and orphaned pipe-read tasks pile up until pages load forever. This shipped as the analyze/optimize permanent-loading bug.
- **RobotSession stream rules**: read stdout to EOF and never cancel reads from `terminationHandler` (fast-exiting processes fire it before buffers drain — the stream hangs forever); use `PipeLines` line streams, not `FileHandle.bytes` (its global IOActor serializes on long-lived pipes and starves other readers); undecodable lines are skipped for forward compatibility, so the `done` event — not EOF — is the completeness signal.
- **Clean scan liveness comes from the preview ledger**: `run_clean_plan` pre-creates a mktemp ledger, injects it via `MOLE_CLEAN_PREVIEW_LEDGER_FILE` (honored non-root only, `-f && ! -L && -O` hygiene), and polls NUL six-tuples. The export file only materializes after the scan; polling it shows nothing for minutes.
- **`bytes == nil` means "size unknown", not 0**: `RobotItem`/`RobotInsight` optionals render as 大小未知/—; sums and `bytes_total` count known sizes only. Never coerce nil to 0 in a display context.
- **`Localizable.xcstrings` must round-trip Xcode's own serialization** (space around colons, sorted keys, 2-space indent). Any other style makes Xcode rewrite the whole file on open, which blocked `git pull` once. New robot task/section ids need `optimize.task.<id>.name/.desc`-style entries or the UI falls back to English.
- **Confirm-list rows must stay direct children of the single top-level LazyVStack** (flattened via nested ForEach — group headers, rows, reveal buttons each their own lazy item). Wrapping an expanded group's rows in any non-lazy container materializes the whole card, and then every invalidation (scroll, checkbox toggle, each animation frame, every per-row hover flip) re-measures hundreds of dual-line truncated Texts per pass — seconds on real hardware while a cold-layout test harness stays green. This shipped twice as a confirm-page hang (2026-08-15 samples: `enqueueHoverUpdateIfNeeded` / bare transaction replay driving whole-tree relayout). Nested LazyVStack is not a fix (no viewport → degenerates to full materialization). The virtualization is also what makes per-row affordances safe: with only ~20–30 viewport rows materialized, each row may carry `pointingCursor`, `.onHover` brighten, and `.help` tooltips — the responder budget is bounded by the viewport, not the item count, and removing these was reverted on user feedback. Both halves are pinned by the transaction-replay phase in `CleanConfirmLayoutPerfTests` (repeated state invalidation must stay cheap per pass); that headless test still cannot catch mouse-event storms, so after touching row structure, sample the live process while sweeping the mouse.
...
- Robot layer changes: run `MOLE_TEST_NO_AUTH=1 bats tests/robot_core.bats tests/robot_apps_plan.bats tests/robot_optimize.bats tests/robot_launchitems.bats tests/robot_updates.bats`, then a read-only e2e smoke `MOLE_TEST_NO_AUTH=1 ./mole robot clean plan` (assert valid NDJSON, progress events during the scan, and a `done` with plan_id).
- App changes: `cd app && xcodegen generate && xcodebuild -scheme Mole -configuration Debug build CODE_SIGNING_ALLOWED=NO`; MoleKit changes also run `cd app/MoleKit && swift test`.
