...
- **Confirm-list rows keep zero per-row hover/tooltip responders beyond the audited set.** Two shipped hangs prove the pattern: hundreds of materialized rows × any per-row `NSTrackingArea` source (`pointingCursor`, `.onHover`, broad `.help`) turns every mouseMoved into responder-graph walks plus layout transactions over hundreds of measured Text rows — the 2026-08-15 sample showed `enqueueHoverUpdateIfNeeded` driving whole-tree relayout seconds per pass. Allowed today: the checkbox cursor, one `.help` on the whitelist shield (design-required consequence copy), and data-critical `.help` on unknown-size/0B/lock elements. Hover-to-brighten is a deliberate design deviation (constant 0.55 opacity instead); do not reintroduce per-row `onHover` state, and note the headless layout perf test cannot catch mouse-event storms — sample the live process instead.
